...
//                Toast.makeText(this, "카메라 권한이 승인됨", Toast.LENGTH_SHORT).show();
                ...
                Snackbar.make(viewPager,"권한이 필요합니다",Snackbar.LENGTH_SHORT)
                        .setAction("설정으로 이동", new View.OnClickListener() {
                            @Override
...
//                Toast.makeText(this, "카메라 권한이 승인됨", Toast.LENGTH_SHORT).show();
        ...
        if (requestCode == 2000) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                // 권한이 부여된 경우 작업 수행
                // 여기에 파일 쓰기 등의 코드를 추가하세요
            } else {
                // 권한이 거부된 경우 사용자에게 알림을 표시하거나 다른 대응을 수행
                Snackbar.make(viewPager,"권한이 필요합니다",Snackbar.LENGTH_SHORT)
                        .setAction("설정으로 이동", new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
